...
interface BadgeProps {
  children: ReactNode;
  color?: 'blue' | 'red' | 'white' | 'green' | 'purple' | 'orange' | 'gray' | 'yellow';
  className?: string;
}
...
export const Badge: React.FC<BadgeProps> = ({ children, color = 'blue', className = '' }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wider border ${colors[color]} inline-block ${className}`}
    >
      {children}
    </span>
  );
